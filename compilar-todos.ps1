...
$servicios = @(
    "vetnova-auth-service",
    "vetnova-catalogo-service",
    "vetnova-inventario-service",
    "vetnova-ventas-service",
    "vetnova-envio-service",
    "vetnova-agenda-service",
    "vetnova-ficha-service",
    "vetnova-soporte-service",
    "vetnova-laboratorio-service",
    "vetnova-facturacion-service",
    "vetnova-reportes-service",
    "vetnova-notificaciones-service"
)
...
foreach ($servicio in $servicios) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "BUILD SUCCESS: $servicio" -ForegroundColor Green
            $resultados += [PSCustomObject]@{ Servicio = $servicio; Estado = "OK" }
        } else {
            Write-Host "BUILD FAILURE: $servicio" -ForegroundColor Red
